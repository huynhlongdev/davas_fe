...
export default function Footer({ data = {} }) {
  ...
  const logoUrl = logo?.media?.url ? `${baseUrl}${logo.media.url}` : null;
  ...
  return (
    <footer className="bg-ink text-sm text-t4">
      <div className="max-w-[1360px] mx-auto px-4 py-15">
        <div className="grid gap-10 md:grid-cols-[320px_1fr]">
          ...
          <div>
            ...
            {address.length > 0 && (
              <ul className="space-y-3">{address.map(renderAddressItem)}</ul>
            )}
          </div>

          {/* Right */}
          <div>{text && <BlocksRenderer content={text} />}</div>
        </div>
      </div>
